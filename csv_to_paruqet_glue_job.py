...
datasource0 = glueContext.create_dynamic_frame.from_catalog(database = "de_youtube_raw_data", table_name = "raw_statistics", transformation_ctx = "datasource0", push_down_predicate = predicate_pushdown)
...
# Convert the DynamicFrame to a DataFrame for coalesce
df = dropnullfields3.toDF().coalesce(1)

# Create a new DynamicFrame from the DataFrame
df_final_output = DynamicFrame.fromDF(df, glueContext, "df_final_output")

# Write the DynamicFrame to S3 with partitioning
datasink4 = glueContext.write_dynamic_frame.from_options(frame = df_final_output, connection_type = "s3", connection_options = {"path": "s3://de-on-youtube-cleanseddata-useast1-dev/youtube/raw_statistics/", "partitionKeys": ["region"]}, format = "parquet", transformation_ctx = "datasink4")
